...
class Canvas(app.Canvas):
    ...
    def __init__(self):
        ...
        # Create vertices
        source_voxel = np.loadtxt('../source_voxel.dat')
        source_emd = np.loadtxt('../source_emd.dat')
        n = source_voxel.shape[0]
        data = np.zeros(2*n, [('a_position', np.float32, 3),
                            ('a_bg_color', np.float32, 4),
                            ('a_fg_color', np.float32, 4),
                            ('a_size', np.float32, 1)])

        v_color = np.empty((n, 4))
        all_color = np.empty((2*n, 4))
        all_points = np.empty((2*n, 3))
        v_color[:,:3] = source_voxel + 0.5
        v_color[:,3] = 0.7
        all_color[:n] = v_color
        all_color[n:] = v_color
    
        source_left = source_voxel.copy()
        source_left[:,0] -= 0.5
        source_right = source_emd.copy()
        source_right[:,0] += 0.5
        all_points[:n] = source_left
        all_points[n:] = source_right
        
        data['a_position'] = all_points
        data['a_bg_color'] = all_color
        data['a_fg_color'] = 0, 0, 0, 1
        data['a_size'] = np.random.uniform(10*ps, 10*ps, 2*n)
        u_linewidth = 1.0
        u_antialias = 1.0
